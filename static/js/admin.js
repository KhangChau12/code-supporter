...
// Thêm vào cuối document.addEventListener('DOMContentLoaded', function() {...});

// Khởi tạo admin tour guide - Chỉ dành cho trang Admin
document.addEventListener('DOMContentLoaded', function() {
    const hasSeenAdminTour = localStorage.getItem('codesupporter_seen_admin_tour');
    ...
    function startAdminTour() {
        ...
        // 1. Giới thiệu trang Admin
        tour.addStep({
            id: 'admin-welcome',
            title: 'Trang quản lý API Keys',
            text: 'Đây là nơi bạn có thể tạo và quản lý các API key để tích hợp Code Supporter vào ứng dụng của mình.',
            buttons: [
                {
                    text: 'Bỏ qua',
        ...
        tour.addStep({
            ...
            text: 'Nhập tên và chọn quyền hạn cho API key của bạn, sau đó nhấn nút "Tạo API Key".',
            attachTo: {
        ...
        tour.addStep({
            ...
            text: 'Di chuyển giữa các tab để xem hướng dẫn tích hợp và phân tích dữ liệu sử dụng API.',
            attachTo: {
                element: '.admin-tabs',
                on: 'bottom'
            },
        ...
        // 4. Danh sách API keys
        const apiKeysList = document.querySelector('.api-keys-list');
        if (apiKeysList) {
            tour.addStep({
                id: 'api-keys-list',
                title: 'Danh sách API keys',
                text: 'Ở đây bạn có thể xem tất cả API keys đã tạo và thông tin của chúng.',
                attachTo: {
                    element: '.api-keys-list',
                    on: 'top'
                },
                buttons: [
                    {
        ...
        tour.addStep({
            id: 'admin-end-tour',
            title: 'Sẵn sàng!',
            text: 'Bạn đã hoàn thành hướng dẫn trang Admin. Hãy bắt đầu tạo API key và tích hợp Code Supporter vào ứng dụng của bạn!',
            buttons: [
                {
                    text: 'Kết thúc',
                    action: tour.complete
        ...
        // Đánh dấu đã xem tour admin
        localStorage.setItem('codesupporter_seen_admin_tour', 'true');
    }
    ...
    if (window.tooltipManager) {
        ...
        // Thêm tooltips cho các phần tử cụ thể
        const apiKeysTable = document.querySelector('.api-keys-table');
        if (apiKeysTable) {
            window.tooltipManager.addTooltip(apiKeysTable, 'Danh sách các API key đã tạo', {
                position: 'top',
                title: 'API Keys'
            });
        }
    ...
    // Bắt đầu tour nếu là lần đầu vào trang admin
    if (!hasSeenAdminTour) {
        // Chờ trang tải xong hoàn toàn
        setTimeout(() => {
            startAdminTour();
        }, 1000);
    }

    // Thêm nút bắt đầu tour
    const userInfo = document.querySelector('.user-info');
    if (userInfo) {
        const tourButton = document.createElement('button');
        tourButton.id = 'start-admin-tour-button';
        ...
        // Thêm vào trước nút theme toggle
        const themeToggle = document.getElementById('theme-toggle');
        if (themeToggle) {
        ...
        // Thêm event listener
        tourButton.addEventListener('click', startAdminTour);
    }
    
    // Thêm tooltips cho các phần tử được tạo động
    const originalLoadApiKeys = window.loadApiKeys || function(){};
    window.loadApiKeys = function() {
        // Gọi hàm gốc
        originalLoadApiKeys();
        
        // Refresh tooltips sau khi load API keys
        setTimeout(() => {
