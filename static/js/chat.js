...
// Thêm vào cuối phần document.addEventListener('DOMContentLoaded', function() {...});
// Tích hợp tooltips và tour guide
if (window.tooltipManager) {
    window.tooltipManager.refresh();
}

// Override hàm addMessageToUI để thêm tooltips cho nút copy code
const originalAddMessageToUI = window.addMessageToUI || function(){};
window.addMessageToUI = function(content, sender) {
    // Gọi hàm gốc nếu đã được định nghĩa
    if (typeof originalAddMessageToUI === 'function') {
        originalAddMessageToUI.call(this, content, sender);
    } else {
        // Nếu không, thực hiện logic mặc định
        const messageDiv = document.createElement('div');
        messageDiv.className = `message ${sender}-message`;
    ...
    // Thêm tooltips cho các nút copy code
    if (sender === 'bot' || sender === 'assistant') {
        setTimeout(() => {
            document.querySelectorAll('pre').forEach(pre => {
                if (!pre.querySelector('.copy-code-btn')) {
                    ...
                    copyBtn.className = 'copy-code-btn';
                    copyBtn.textContent = 'Copy';
                    copyBtn.setAttribute('data-tooltip', 'Sao chép đoạn mã này');
                    copyBtn.setAttribute('data-tooltip-position', 'top');
            ...
            // Refresh tooltips sau khi thêm nút copy
...
// Thêm vào sau mỗi hàm render hoặc thêm phần tử mới
function renderConversationList() {
    // Code hiện có...
